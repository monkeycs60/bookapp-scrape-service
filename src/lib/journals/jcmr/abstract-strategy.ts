...
export const jcmrAbstractStrategy = async (page: Page, url: string) => {
	...
	return page.evaluate(() => {
		...
		const authorsDetails = allAuthors
			.map((author) => {
				const givenName = author
					.querySelector('span[property="givenName"]')
					?.textContent?.trim();
				const familyName = author
